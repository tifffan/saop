...
switch graph
    ...
    case 'saylr4'
        ...
        G=gsp_graph(A);
        G.L=G.L./1400;
    case 'gr900'   % positive definite, G.lmin=0.06
...
mop_param.grid_order=100;
mop_param.absc_method='warp'; %change back
mop_param.weights_method='pdf';
[absc_wch,weights_wch]=gen_absc_weights(G,K,mop_param);
[ab_wch,Pi_wch]=matrix_adapted_ortho_poly(absc_wch,weights_wch,K);
...
mop_param.weights_method='pdf';
[absc_linear,~]=gen_absc_weights(G,K,mop_param);
%[p_weighted_lsc,s_weighted_ls,mu_weighted_ls]=weighted_polyfit(absc_linear,f(absc_linear),G.spectrum_pdf_approx(absc_linear),K);
mat_absc_linear=absc_linear.^(0:K);
p_weighted_lsc=lscov(mat_absc_linear,f(absc_linear),G.spectrum_pdf_approx(absc_linear));
p_weighted_lsc=flipud(p_weighted_lsc);
%[p_weighted_lsc,s_weighted_ls,mu_weighted_ls]=weighted_polyfit(absc,f(absc),weights,K);
...
%hpf=@(x) (x>=gi(G.lmax*9/10)); % check if lanczos performs bad when b is
%high pass filtered
...
cheb_table=table(poly_orders,cheb_sup_err,cheb_sq_err,cheb_anmse)
leg_table=table(poly_orders,leg_sup_err,leg_sq_err,leg_anmse)
lanc_table=table(poly_orders,lanc_sup_err,lanc_sq_err,lanc_anmse)
warp_table=table(poly_orders,warp_sup_err,warp_sq_err,warp_anmse)
warp_ls_table=table(poly_orders,warp_ls_sup_err,warp_ls_sq_err,warp_ls_anmse)
spec_table=table(poly_orders,spec_sup_err,spec_sq_err,spec_anmse)
spec_warp_table=table(poly_orders,spec_warp_sup_err,spec_warp_sq_err,spec_warp_anmse)
spec_spline_table=table(poly_orders,spec_spline_sup_err,spec_spline_sq_err,spec_spline_anmse)
spec_wch_table=table(poly_orders,spec_wch_sup_err,spec_wch_sq_err,spec_wch_anmse)
weighted_ls_table=table(poly_orders,weighted_ls_sup_err,weighted_ls_sq_err,weighted_ls_anmse)
...
set(plot4, {'MarkerFaceColor'}, ninecolors); 
hold on;
legend(plot4,'Chebyshev','Legendre','Lanczos','Warped Chebyshev','Warped LS',...
    'Spectrum-Adapted Ortho. Poly.','Spectrum-Adapted w/ Warped Grid','Spectrum-Adapted w/ Spline',...
    'Spectrum-Adapted w/ Warped Cheby Grid','Weighted LS','Location','Northwest');  
xlabel('K');
