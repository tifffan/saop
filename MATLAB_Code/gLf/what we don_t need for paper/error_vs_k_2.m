...
switch graph
   ...
   case 'saylr4scaled'
    ...
    case 'gr3030'   % positive definite, G.lmin=0.06
        ...
        G.N=size(G.L,1);
    case 'cage7'
        load('/Users/lfan/Documents/MATLAB/git/spectral-warping/MATLAB_Code/Data/cage7.mat');
        G=struct;
        G.L=Problem.A;
        G.L=(G.L+G.L')/2;
        G.N=size(G.L,1);
    case 'cage9'
        ...
        G.L=(G.L+G.L')/2;
        G.N=size(G.L,1);
    case 'cdde1'   % original data not symmetric, take (L+L')/2
   ...
   case 'si2' % positive definite
    ...
    case 'nd3k' % N=9000
        ...
        G.L=G.L./20;
    case 'barth4' % N=6019
        load('/Users/lfan/Documents/MATLAB/git/spectral-warping/MATLAB_Code/Data/barth4.mat');
        A=Problem.A;
        A = A - diag(diag(A)); 
        G=gsp_graph(A);
end
...
figure;
plot2=semilogy(G2.e,[abs(errors_cheb),abs(errors_lanczos),abs(errors_cheb_warped),abs(errors_spec_adapted_ortho),abs(errors_weighted_ls)],'-o','LineWidth',1,'MarkerSize',10);
set(gca,'FontSize',24);
set(plot2, {'MarkerFaceColor'}, get(plot2,'Color')); 
hold on;
plot(G2.e,ones(G.N,1),'xk','LineWidth',2,'MarkerSize',6);
legend(plot2,'Chebyshev','Lanczos','Warped Interpolation','SAOP','Weighted LS','Location','SouthEast');  
xlabel('\lambda');
ylabel('$$|f(\lambda)-p_{10}(\lambda)|$$','Interpreter','latex');
grid on;

% delta=(G.lmax-G.lmin)/10000;
% xx=G.lmin:delta:G.lmax;
% xx=xx';
% yy=f(xx);
% 
...
% yy_weighted_ls=polyval(p_weighted_lsc,xx,s_weighted_ls,mu_weighted_ls);
% 
% dom=domain(G.lmin,G.lmax);
% p_lanc_cheb=chebfun.interp1(e,f(e),dom);
% p_lanc_chebc=p_lanc_cheb.coeffs;
% p_lanc_chebc(1)=p_lanc_chebc(1)*2;
% yy_lanczos=gsp_cheby_eval(xx,p_lanc_chebc,[G.lmin,G.lmax]);
% 
% [p_lanc,s_lanc,mu_lanc]=polyfit(e,f(e),K);
% yy_lanczos=polyval(p_lanc,xx,s_lanc,mu_lanc); 
% 
% figure;
% plot2=semilogy(xx,[abs(yy-yy_cheb),abs(yy-yy_lanczos),abs(yy-yy_cheb_warped),abs(yy-yy_spec_adapted_ortho),abs(yy-yy_weighted_ls)],'LineWidth',4);
% set(gca,'FontSize',24);
% set(plot2, {'MarkerFaceColor'}, get(plot2,'Color')); 
% hold on;
% sz=100;
% scatter(G2.e,abs(errors_cheb),sz,'Marker','o','MarkerEdgeColor',[ 153 204 255]/256,'MarkerFaceColor',[ 153 204 255]/256);
% scatter(G2.e,abs(errors_lanczos),sz,'Marker','o','MarkerEdgeColor',[255 179 153]/256,'MarkerFaceColor',[255 179 153]/256);
% plot(G2.e,ones(G.N,1),'xk','LineWidth',2,'MarkerSize',6);
% legend(plot2,'Chebyshev','Lanczos','Warped Interpolation','SAOP','Weighted LS','Location','SouthEast');  
% xlabel('\lambda');
...
switch btype
    ...
    case 'low'
        ...
        BBB=(1/sqrt(G.N))*rand(G.N,num_tests);
        BB=G2.U*BBB;
        f2=@(x) (x<=gi(G.lmax*1/4)); 
        BB=gsp_filter(G,f2,BB);
    case 'high'
        num_tests=50;
        BBB=(1/sqrt(G.N))*rand(G.N,num_tests);
        BB=G2.U*BBB;
        f2=@(x) (x>=gi(G.lmax*3/4)); 
        BB=gsp_filter(G,f2,BB);
    otherwise
...
plot6=semilogy(poly_orders,[cheb_sq_err/ysq,lanc_sq_err/ysq,warp_sq_err/ysq,...
    spec_sq_err/ysq,weighted_ls_sq_err/ysq],'-o','LineWidth',1,'MarkerSize',10);
set(gca,'FontSize',24);
