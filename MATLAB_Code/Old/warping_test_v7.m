...
yy_cheb_warped=gsp_cheby_eval(xx,p_warped_c,[0,lmax_est]); %ppval(p_warped,xx); 
...
figure;
plot3=semilogy(G.e,[abs(errors_cheb),abs(errors_leg),abs(errors_cheb_warped),abs(errors_spec_adapted_ortho),abs(errors_ls),abs(y-y_lanczos)],'-o','LineWidth',1,'MarkerSize',10);
set(gca,'FontSize',20)
set(plot3, {'MarkerFaceColor'}, get(plot3,'Color')); 
hold on;
plot(G.e,ones(G.N,1),'xk','LineWidth',2,'MarkerSize',6);
legend(plot3,'Chebyshev','Legendre','Warped Interpolation','Spectrum-Adapted Ortho. Poly.','Discrete LS','Lanczos','Location','SouthEast');  
xlabel('\lambda');
ylabel('$$|\tilde{g}(\lambda)-g(\lambda)|$$','Interpreter','latex');
grid on;
