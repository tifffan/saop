...
%
%   Url: https://epfl-lts2.github.io/gspbox-html/doc/filters/utils/gsp_mono_cubic_warp_fn.html
...
for ii=1:num_pts_to_interpolate
    if x0(ii) < x(1)
        ...
        derivative_values(ii)=0;
    elseif x0(ii) > x(num_pts)
        %interpolated_values(ii)=1;
        derivative_values(ii)=0;
    else
    ...
    derivative_values(ii) = 1/h *(y(lower_ind)*(6*t^2-6*t) + h*m(lower_ind)*(3*t^2-4*t+1) + y(lower_ind+1)*(-6*t^2+6*t) + h*m(lower_ind+1)*(3*t^2-2*t));
end
end
